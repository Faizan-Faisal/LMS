    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-lmsblue-dark to-lmsblue py-0 px-4">
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-8">
        {loginOptions.map((option) => (
          <div
            key={option.title}
            className="bg-white rounded-2xl shadow-xl p-8 flex flex-col items-center hover:scale-105 transition-transform duration-200 group"
          >
            {option.icon}
            <h3 className="text-2xl font-bold text-gray-800 mb-2 text-center">{option.title}</h3>
            <p className="text-gray-500 text-center mb-6">{option.description}</p>
            <button
